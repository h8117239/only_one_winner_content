...
function OnBuildingExchanged( msg )
{
	$.Msg( "OnBuildingExchange: ", msg );
	$.GetContextPanel().SetHasClass( "item_will_spawn", false );
	$.GetContextPanel().SetHasClass( "item_has_spawned", true );
	// $.GetContextPanel().SetHasClass( "continue_buttons", true);
	// $.GetContextPanel().SetHasClass( "cancel_buttons", true);
	$( "#AlertMessage_Chest" ).html = true;
	$( "#AlertMessage_Delivery" ).html = true;
	$( "#AlertMessage_Chest" ).text = $.Localize( "#"+msg.building );
	$( "#AlertMessage_Delivery" ).text = $.Localize( "#BeOccupied"+msg.team );
	GameUI.PingMinimapAtLocation( msg.pos );				
	$.Schedule( 3, ClearItemSpawnMessage );
	// $.Schedule( 3, ClearButtonMessage );
}


function OnGameRoundEnd(msg)
{
	$.GetContextPanel().SetHasClass( "continue_buttons", true);		
	$.Schedule( 3, ClearButtonMessage );	
}
...
{
	$.GetContextPanel().SetHasClass( "continue_buttons", false);
}
...
function OnCancel()
{
	GameEvents.SendCustomGameEventToServer( "OnContinueOrCancel",  {select: -1} );
	ClearButtonMessage()
}

function OnGameContinue( msg )
{
	$.Msg( "OnGameContinue: ", msg );
	$.GetContextPanel().SetHasClass( "item_will_spawn", true );
	$.GetContextPanel().SetHasClass( "item_has_spawned", false );
	$( "#AlertMessage_Chest" ).html = true;
	$( "#AlertMessage_Delivery" ).html = true;
	$( "#AlertMessage_Chest" ).text = $.Localize( "#hardup"+msg.hard );
	$( "#AlertMessage_Delivery" ).text = $.Localize( "#GameContinue" );
